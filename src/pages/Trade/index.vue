...
export default {
  ...
  updated() {
    if (this.userAddress.find((item) => item.isDefault == "1")) {
      console.log(this.defaultUser);
    } else {
      this.userAddress[0].isDefault = "1";
    }
  },
  methods: {
    //修改默认地址
    ...
    submitInfo() {
      ...
      this.$store.dispatch("submitInfo", { tradeNo, data }).then((res) => {
        //将来提交订单成功【订单ID生成】，路由跳转pay页面，进行支付
        this.$router.push({
          path: "/pay",
          query: { orderId: this.$store.state.trade.payId },
        });
      });
    },
  },
  ...
  computed: {
    ...mapState({
      userAddress: (state) => {
        return state.trade.userAddress;
      },
      tradeInfo: (state) => state.trade.tradeList,
    }),
    defaultUser() {
