...
export default {
  ...
  methods: {
    ...
    handler: throttle(function (type, num, cartItem) {
      switch (type) {
        case "add":
          num = 1;
          break;
        ...
        case "change":
          if (num < 1 || isNaN(num)) {
            ...
            num = parseInt(num) - cartItem.skuNum;
          }
          break;
      }
      this.$store
        .dispatch("addToCart", { skuId: cartItem.skuId, skuNum: num })
        .then((res) => {
          this.getData();
        });
    }, 1000),
    ...
    deleteCartById(cartItem) {
      this.$store.dispatch("deleteCartById", cartItem.skuId).then((res) => {
        this.getData();
      });
    },
  ...
  computed: {
    ...
    totalPrice() {
      return this.cartInfoList
        ...
        .reduce((pre, item) => (pre += item.skuNum * item.cartPrice), 0);
    },
    // isCartChecked() {
